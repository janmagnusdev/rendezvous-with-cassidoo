...
export const getFibonacciLike = (
  ...
  if (first < 0 || second < 0 || length < 0)
    throw new Error(fibonacciErrors.Negative);
...
export const isFibonacciLike = (sequence: number[]): boolean => {
  if (
    sequence.length < 3 ||
    sequence.some((value) => Number.isNaN(value) || value < 0)
  )
    return false;
  ...
  return sequence
    ...
    .every(
      (value, index) =>
        ...
        value === sequence[index] + sequence[index + 1]
    );
};
