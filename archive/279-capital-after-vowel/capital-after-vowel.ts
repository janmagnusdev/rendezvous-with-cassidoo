...
export const capitalAfterVowel = (string: string): string =>
  [...string]
    .map((letter, index) =>
      isConsonant(letter) && isLastLetterVowel(string, index)
        ? letter.toUpperCase()
        : letter
    )
    .join("");

const regexConverter = (full: string, match: string) =>
  full.replace(match, match.toUpperCase());

export const capitalAfterVowelRegex = (string: string): string =>
  string.replace(
    new RegExp(`[${VOWELS}]\\s*([b-df-hj-np-tv-xz])`, "gi"),
    regexConverter
  );
